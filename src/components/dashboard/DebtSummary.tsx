  ...
  return (
    <div className="card h-full">
      <div className="px-5 py-4 border-b border-line flex items-center justify-between">
        <span className="text-[9px] font-mono tracking-[0.12em] uppercase text-muted">Borç Takibi</span>
        <Link href="/debts" className="text-[9px] font-mono tracking-wide uppercase text-muted hover:text-ink transition-colors">
          Tümü →
        </Link>
      </div>
      ...
      <div className="px-5 py-4 border-b border-line flex items-baseline gap-2">
        <span className="text-2xl font-black tabular tracking-tight text-ink">
          {formatCurrency(totalOwed)}
        </span>
        <span className="text-[10px] font-mono text-muted uppercase">toplam borç</span>
      </div>

      {/* Due soon */}
      {dueSoon.length === 0 ? (
        <div className="px-5 py-4 text-xs text-muted">30 gün içinde vadesi gelen ödeme yok.</div>
      ) : (
            ...
            return (
              <div key={debt.id} className="px-5 py-3">
                <div className="flex items-start justify-between gap-2">
                  <div>
                    <div className="text-xs font-medium">{debt.name}</div>
                  ...
                  <div className="text-right flex-shrink-0">
                    <div className="text-xs font-mono tabular text-ink">
