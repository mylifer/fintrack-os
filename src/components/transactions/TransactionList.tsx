...
interface Props {
   ...
   *  için de ileriye dönük projeksiyon gösterir (defterde olmadıkları için aksi
   *  halde boş kalırlar). */
  plannedTxs?: Transaction[]
  /** Kaydırma kabının yükseklik sınıfı. Varsayılan viewport'a göre sabittir;
   *  liste görünüm seçicili bir kabuğa gömülüyse çubuğun payı düşülmüş bir
   *  değer geçilir. */
  heightClass?: string
}
...
export function TransactionList({
  ...
  const rows = useMemo<Row[]>(() => {
    ...
    // Gün kartı görünümü de aynı ayrımı paylaşır (views/shared).
