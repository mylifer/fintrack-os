 ...
 * tıklanınca sıralama değişir. Çok alıcıyı yukarıdan aşağı KIYASLAMAK için.
 */
export function TableView({ rows, query, sort, onSort, onEdit, onArchive }: RecipientViewProps) {
  return (
      ...
      <table className="w-full">
        <thead className="sticky top-0 z-10 bg-card">
          <tr className="border-b border-border text-[10px] font-semibold uppercase tracking-wide text-muted-foreground">
            <SortTh id="name"   sort={sort} onSort={onSort} className="text-left pl-4">Alıcı</SortTh>
        ...
        <tbody>
          {rows.map(row => {
            const { person, flowCount, pendingCount, expense, share, lastDate } = row
