// ── Legacy icon migration map (shared) ──────────────────────────────────────
// Single source of truth for converting legacy icon identifiers → Tabler kebab
// names + a meaningful color. Covers every historical format:
//   • original emoji (era: first commits, e.g. 🛒 🏠 🚗)
//   • Lucide PascalCase (era: commit 1b769c8)
//   • noto:/Iconify (era: commits 77f2cde / db v6)
// Used by categories.store.ts load()/initDefaults() and the Dexie v6 upgrade in
// lib/db/index.ts. Lookups are by exact key, so the superset is safe everywhere
// (the v6 migration only ever encounters the noto: subset).
export const NOTO_TO_TABLER: Record<string, { icon: string; color: string }> = {
...
/** The legacy default category color; icon-migration only overrides color when a
 *  category still carries this exact value (never clobbers a user-picked color). */
export const LEGACY_COLOR = '#6B8F80'
