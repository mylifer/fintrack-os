...
export async function readSnapshot(): Promise<BackupData> {
  const [accounts, transactions, categories, budgets, debts, investmentTransactions, people, recurringTransactions, paymentSchedules] =
    await Promise.all([
      ...
      db.people.toArray(), db.recurringTransactions.toArray(), db.paymentSchedules.toArray(),
    ])
  return {
    ...
    paymentSchedules:       paymentSchedules.filter(isLive),
  }
}
