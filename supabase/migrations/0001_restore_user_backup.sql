-- ============================================================================
-- restore_user_backup(payload jsonb, target_user_id uuid)
--
-- Atomically replaces ALL of a user's data with the contents of a backup file.
-- Everything below runs inside the single transaction of the RPC call: if any
-- statement fails, the whole restore is rolled back (no partial-sync risk).
--
-- payload shape (keys match the app's backup file `data` object):
--   { accounts, transactions, categories, budgets, debts,
--     investmentTransactions, people, recurringTransactions }  -- each a JSON array
--
...
begin
  ...
  -- (unknown keys such as accounts.balance are ignored). We select an explicit
  -- column list so any server-managed defaulted columns are left untouched, and
  -- force user_id to the caller. A missing/empty array yields zero rows.
  -- categories are inserted in a single statement, so the self-referential
  -- parentId FK is satisfied regardless of element order.
  ...
  insert into public.transactions
    (id, type, amount, currency, date, "accountId", "toAccountId", "categoryId", icon, description, notes, tags, merchant,
     "familyMemberId", "recipientId", "isInstallment", "installTotal", "installIndex", "installGroupId", "debtId",
     "createdAt", "updatedAt", user_id)
  select r.id, r.type, r.amount, r.currency, r.date, r."accountId", r."toAccountId", r."categoryId", r.icon, r.description, r.notes, r.tags, r.merchant,
