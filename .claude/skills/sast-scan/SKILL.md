...
name: sast-scan
description: >-
  Run a full SAST (static application security testing) sweep over this codebase.
  Orchestrates the sast-* skills: first architecture analysis, then all
  vulnerability detectors in parallel, then a consolidated report. Read-only —
  only writes markdown reports under sast/. Use when asked to "run a vulnerability
  scan", "find vulnerabilities", "güvenlik taraması yap", or run a SAST audit.
---

# SAST Security Assessment (orchestrator)

Your goal is to identify security vulnerabilities in the codebase in the current directory. This is a **read-only** assessment: the only files you may create are the markdown reports under `sast/`. Never modify application code or user data during the scan.
